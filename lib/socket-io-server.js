module.exports = function Server(expressInstance, sessionStore) {
  ...
  var   Bidder = require("../bidder").Bidder;
    ...
    if (user) {
      ...
      if (bidder) {
        console.log('reconnected ' + name + ' to bidder. client.id:' + client.id);
        bidder.new_client(client);

      } else {
        console.log('new bidder for  ' + name + ' client.id:' + client.id);
        bidder = new Bidder({ name: name, sid: client.handshake.sid, client: client, image: image});
      }
